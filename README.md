...
1. On the dashboard, use **Shared display schedule** to enter a GitHub fine-grained personal access token (or classic token) with permission to create and update Gists. The dashboard stores that token only in that browser's `localStorage`; it is never included in generated links.
